...
def movie_data():
    XGB_model = XGBClassifier(booster='gbtree', colsample_bytree=0.6,
                              importance_type='gain', max_depth=5,
                              min_child_weight=1, validate_parameters=1)
    ...
    prediction_y = XGB_model.predict(cv.transform(X_test))

    score_1 = accuracy_score(y_test, prediction_y)

    print(score_1)

    y_true, y_pred = y_test, XGB_model.predict(X_test_trans)
    print(classification_report(y_true, y_pred))

    fig, axes = plt.subplots(3, 2, figsize=(10, 15))

    title = r"Learning Curves (XGBoost - Dataset 1)"
...
def chess_game_data():
    XGB_model = XGBClassifier(booster='gbtree', colsample_bytree=0.6,
                              importance_type='gain', max_depth=15,
                              min_child_weight=1, validate_parameters=1)
    ...
    X_train, X_test, y_train, y_test = train_test_split(data_X.fillna(0), data_y, test_size=0.3, random_state=0)
    ...
    title = r"Learning Curves (XGBoost - Dataset 2)"
    cv = ShuffleSplit(n_splits=10, test_size=0.2, random_state=0)
    estimator = XGB_model
